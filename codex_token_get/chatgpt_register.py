...
PROXY_POOL = _CONFIG.get("proxy_pool", [])
PROXY_API = _CONFIG.get("proxy_api", "")
...
def _fetch_proxy_from_api(api_url: str) -> str:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            proxy_str = resp.read().decode("utf-8").strip()
        ...
        with _print_lock:
            print(f"[Proxy] 从 API 获取代理失败: {e}")
    return ""


def _get_next_proxy() -> str:
    """获取下一个代理 IP，支持三种模式:
    1. proxy_api: 每次从 API 动态获取新代理（推荐，适配大多数代理商）
    2. proxy_pool: 从静态代理列表中轮换
    3. proxy: 使用单个固定代理
    """
    global _proxy_index

    # 优先级 1: 代理 API（每次请求新 IP）
    if PROXY_API:
        proxy = _fetch_proxy_from_api(PROXY_API)
        if proxy:
            with _print_lock:
                print(f"[Proxy] API 获取代理: {proxy}")
            return proxy
...
def _supported_impersonates():
    """读取当前 curl_cffi 版本可用的 impersonate 列表。"""
...
_SUPPORTED_IMPERSONATES = _supported_impersonates()
_FALLBACK_IMPERSONATE = "chrome124" if "chrome124" in _SUPPORTED_IMPERSONATES else "chrome"
_ACTIVE_CHROME_PROFILES = [
    p for p in _CHROME_PROFILES if p.get("impersonate") in _SUPPORTED_IMPERSONATES
]
if not _ACTIVE_CHROME_PROFILES:
    _ACTIVE_CHROME_PROFILES = [
        {
            "major": 124,
            "impersonate": _FALLBACK_IMPERSONATE,
            "build": 6367,
            "patch_range": (100, 250),
...
def _random_chrome_version():
    profile = random.choice(_ACTIVE_CHROME_PROFILES)
    major = profile["major"]
    build = profile["build"]
...
class ChatGPTRegister:
    ...
    def _safe_json(response, context):
        try:
            ...
            raise Exception(f"{context} 返回非 JSON: status={response.status_code}, body={body[:240]}")

    # ==================== DuckMail 临时邮箱 ====================

    def _create_duckmail_session(self):
        """创建 DuckMail 请求会话（不走代理，避免 SSL 错误）"""
        session = curl_requests.Session()
        session.headers.update({
            "User-Agent": self.ua,
            "Accept": "application/json",
        ...
        # DuckMail 不需要代理，直连即可，走代理反而容易 SSL 握手失败
        return session
    ...
    def create_temp_email(self):
        ...
        try:
            ...
            if token_res.status_code == 200:
                token_data = self._safe_json(token_res, "DuckMail /token")
                mail_token = token_data.get("token")
                if mail_token:
                    return email, password, mail_token
    ...
    def get_csrf(self) -> str:
        ...
        data = self._safe_json(r, "Get CSRF")
...
def _register_one(idx, total, proxy, output_file):
    ...
    try:
        # 每次注册使用不同的代理 IP
        actual_proxy = _get_next_proxy() or proxy
        reg = ChatGPTRegister(proxy=actual_proxy, tag=f"{idx}")
        if actual_proxy:
            reg._print(f"[Proxy] 当前使用代理: {actual_proxy}")
...
def main():
    print("=" * 60)
    print("  ChatGPT 批量自动注册工具 (DuckMail 临时邮箱版) - 无头模式")
    print("=" * 60)
    ...
    proxy_mode = "API" if PROXY_API else ("池轮换" if PROXY_POOL else ("固定" if proxy else "无"))
    print(f"[Info] 代理模式: {proxy_mode}")
    if PROXY_API:
        print(f"[Info] 代理API: {PROXY_API}")
    if PROXY_POOL:
        print(f"[Info] 代理池数量: {len(PROXY_POOL)}")
    if proxy:
        print(f"[Info] 固定代理: {proxy}")
